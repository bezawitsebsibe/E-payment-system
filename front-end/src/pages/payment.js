...
const Payment = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/Users/serviceNo/${serviceNo}`);
        ...
        const userBillResponse = await axios.get(`http://localhost:3001/bills/findOne`, {
          params: {
            userId: userId,
  ...
  const sendVerificationEmail = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:3001/Users/verifyUser/${userId}/${code}`);
  ...
  const handleBankAccountSubmit = async () => {
    try {
      ...
      const response = await axios.put(`http://localhost:3001/bills/${userbill.id}`, {
        billStatus: "paid",
